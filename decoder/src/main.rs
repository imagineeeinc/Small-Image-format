...
use colorsys::{Rgb};
use image::{GenericImageView, GenericImage, ImageBuffer, RgbImage};
use std::path::Path;
use std::io::Write;
use std::fs;
...
fn main() {
    ...
    println!("\n\tWelcome to SIF Decoder!");

    if args.len() > 1 {
        println!("\n\tInput file: {:?}", args[0]);
        println!("\tOutput file: {:?}", args[1]);
        let data = decoder(args[0].to_string());
        // let imgdata = &data[0];
        // let exif = &data[1];
...
pub fn help() {
    println!("\n\tUsage: sif-decoder [options] <input file> <output file>");
    println!("\n\tOptions:");
...
pub fn decoder(input: String) -> Result<()> {// -> Vec<String>
    let imgpath = Path::new(&input[..]);
    print!("\tReading image file: {}\n", imgpath.to_str().unwrap());
    let img = fs::read_to_string(input)
    .expect("Something went wrong reading the image file");
    let mut dimensions = &img[..img.chars().position(|c| c == '}').unwrap()+1];
    // dimensions = &dimensions[..];
    // println!("{}", dimensions);
    let dimensions: Result<()> = serde_json::from_str(dimensions)?;
    println!("{:?}", dimensions);
    Ok(())
    // //let (width, height) = img.dimensions();
    // let mut img_data: RgbImage = ImageBuffer::new(512, 512);
